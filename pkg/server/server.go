...
const (
	...
	defaultBossLocation string = "/dev/"
...
func (s *BossServer) Run() error {
	...
	err = syscall.Unlink(DevicePluginPath + bossSocket)
	if err != nil && !os.IsNotExist(err) {
		return err
	}

	l, err := net.Listen("unix", DevicePluginPath+bossSocket)
	if err != nil {
		return err
	}
	...
	conn, err := s.dial(bossSocket, 5*time.Second)
	if err != nil {
		return err
	}
	conn.Close()
...
func (s *BossServer) RegisterToKubelet() error {
	...
	req := &pluginapi.RegisterRequest{
		...
		Endpoint:     path.Base(DevicePluginPath + bossSocket),
...
// Manager
func (s *BossServer) GetDevicePluginOptions(ctx context.Context, e *pluginapi.Empty) (*pluginapi.DevicePluginOptions, error) {
...
func (s *BossServer) ListAndWatch(e *pluginapi.Empty, srv pluginapi.DevicePlugin_ListAndWatchServer) error {
...
func (s *BossServer) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
	...
	for _, req := range reqs.ContainerRequests {
		...
		resp := pluginapi.ContainerAllocateResponse{
			Envs: map[string]string{
				"BOSS_DEVICES": strings.Join(req.DevicesIDs, ","),
			},
		}
...
func (s *BossServer) PreStartContainer(ctx context.Context, req *pluginapi.PreStartContainerRequest) (*pluginapi.PreStartContainerResponse, error) {
	log.Infoln("PreStartContainer called")
	return &pluginapi.PreStartContainerResponse{}, nil
}

// listDevice 从节点上发现设备
func (s *BossServer) listDevice() error {
	dir, err := ioutil.ReadDir(defaultBossLocation)
	if err != nil {
		return err
	}
...
func (s *BossServer) watchDevice() error {
...
func (s *BossServer) dial(unixSocketPath string, timeout time.Duration) (*grpc.ClientConn, error) {
